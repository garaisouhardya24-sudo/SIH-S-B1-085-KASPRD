...
def ensure_request(c, farmer_id, state, centre, crops):
    if not crops: return
    exists = c.execute(
        "SELECT id FROM requests WHERE farmer_id=? AND status='Pending'", (farmer_id,)).fetchone()
    if exists: return
    crop = crops[0]
...
def add_shipment(s: Shipment):
    ...
    c.execute("UPDATE farmers SET centre=?,crops=? WHERE id=?",
              (s.centre, json.dumps(crops), r["id"]))
...
def history(token: str):
    c = conn(); rows = c.execute(
        "SELECT * FROM shipments WHERE farmer_token=? ORDER BY id DESC", (token,)).fetchall(); c.close()
...
def update_request(request_id: int, body: RequestResponse):
    if body.status not in {"Accepted", "Declined",
                           "Pending"}: raise HTTPException(400, "Invalid request status")
    c = conn(); c.execute("UPDATE requests SET status=? WHERE id=?",
                          (body.status, request_id)); c.commit(); c.close()
...
def worker_state():
    ...
    grade_rows = c.execute(
        "SELECT farmer_token,crop,grade,remarks,graded_date FROM grades ORDER BY id DESC").fetchall()
...
def complete_appointments(farmer_id: int):
    c = conn(); c.execute("UPDATE appointments SET status='Completed' WHERE farmer_id=?",
                          (farmer_id,)); c.commit(); c.close()
    return {"message": "Appointments completed"}


@app.post("/api/payments")
def create_payment(p: PaymentIn):
    if not 9 <= len(p.account) <= 18 or not p.account.isdigit(
    ): raise HTTPException(400, "Invalid account")
    if not __import__("re").match(r"^[A-Z]{4}0[A-Z0-9]{6}$",
                                  p.ifsc.upper()): raise HTTPException(400, "Invalid IFSC")
